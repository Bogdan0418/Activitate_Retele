...
class FTPClient:
    ...
    # === Functie utilitara pentru a selecta un fisier de pe server ===
    def get_server_file_choice(self):
        """Helper to fetch files from server and let user choose one"""
        response = self.send_command({'command': 'list_files'})
        if response['status'] != 'success':
            print(f"✗ Failed to get files: {response['message']}")
            return None
    ...
    def read_file(self):
        ...
        filename = self.get_server_file_choice()
        if not filename:
            return
    ...
    def download(self):
        ...
        if response['status'] == 'error':
            ...
            # Salvam continutul primit pe local
            filepath = os.path.join(LOCAL_FILES_DIR, filename)
            try:
                with open(filepath, 'w') as f:
                    f.write(response.get('content', ''))
                print(f"✓ File '{filename}' downloaded successfully to '{LOCAL_FILES_DIR}/'")
            except Exception as e:
                print(f"✗ Error saving downloaded file: {str(e)}")
    ...
    def edit_file(self):
        ...
        print("Note: This will overwrite the existing content.")
        new_content = input(f"Enter new content for '{filename}': ").strip()
    ...
    def see_file_operation_history(self):
        ...
        if response['status'] == 'error':
            ...
            if not history:
                print("No history operations recorded yet.")
            else:
    ...
    def show_menu(self):
        ...
        print("3. Upload File")
        print("4. Rename File (Server)       [IMPLEMENTED]")
        print("5. Read File (Server)         [IMPLEMENTED]")
        print("6. Download File              [IMPLEMENTED]")
        print("7. Edit File (Server)         [IMPLEMENTED]")
        print("8. See File Operation History [IMPLEMENTED]")

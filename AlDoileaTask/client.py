...
while True:
    ...
    if comanda == 'EXIT':
        ...
        if not este_conectat:
            print("EROARE LOCALA: Trebuie sa te conectezi mai intai (foloseste CONNECT).")
            continue

        if len(parti) < 2 or not parti[1].strip():
            print("EROARE LOCALA: Lipseste mesajul. Sintaxa corecta: PUBLISH <text_mesaj>")
            continue
        ...
        if not este_conectat:
            print("EROARE LOCALA: Trebuie sa te conectezi mai intai.")
            continue

        if len(parti) < 2 or not parti[1].strip().isdigit():
            print("EROARE LOCALA: ID invalid. Trebuie sa fie un numar. Sintaxa: DELETE <id>")
            continue
        ...
        if not este_conectat:
            print("EROARE LOCALA: Trebuie sa te conectezi pentru a vedea lista de mesaje.")
            continue
